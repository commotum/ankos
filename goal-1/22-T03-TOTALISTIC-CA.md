...
| Complete sum table/cardinality | PRINCIPLED EXTENSION | Table arity must derive as `M=1+(k-1)(2r+1)` and validate every output in `A`; the current schema supplies no sum-case set object. |
...
| `_lookup_index` | PARAMETERIZATION for one sum channel | One channel happens to pass sum through unchanged, but the helper bit-shifts multiple channels as binary positions (`rollout.py:811-822`) rather than using typed case sets or mixed radices. |
...
3. Extend the synthesis-selected `src/ca/rule_tables.py` with a typed aggregate-case set and immutable complete table `U[0..M-1]`. Reuse a generic finite-table carrier only if exhaustive-context and aggregate-sum case tags cannot be confused. Validate `M=1+(k-1)q`, every output, leading zeros, stable identity, and lossless structural serialization.
...
- Remove the assumption that every summarized channel is binary or that spatial output is one rule-ID bit. A one-channel sum may index directly, but all case sets and table outputs remain typed.
