...
The evidence/search closure and conformance fixtures remain valid. The Goal 2 handoff has been reduced to a strict preset over the generic fixed-lattice local-rule construction.
...
| Broad `TOTALISTIC` category | PARAMETERIZATION / SEMANTIC MISMATCH | The schema has an aggregate-then-table shape (`simple_programs.md:1964-2008`) but treats active counts, numeric sums, and color histograms as sibling examples without typed case sets. T05 requires only the exact numeric sum; a histogram distinguishes contexts that T05 merges. |
| Exact sum image and complete table | PRINCIPLED EXTENSION inherited from G2-T03 | The preset derives `0..3(k-1)` and `M=3k-2`; current documentation has no validated aggregate-case set, complete-table invariant, structural identity, or row order. |
