...
`42-T35-source-oracle.py` is the frozen fail-closed textual evidence record. Its 42 redundant query lanes produce a 103-line union at `70 pre-Index / 33 actual Index`. It retains 127 lines partitioned `46 native / 63 relation / 18 control`, including 64 governed continuations, excludes seven false positives, routes the 33 Index rows as `19 core/observer / 10 arithmetic-emulation / 4 ordered-fraction`, and leaves zero unresolved mechanics. Six independent broad Index lanes reproduce exactly the same 33-row boundary rather than merely partitioning whatever the main queries happened to find. The source-model record has 40 clauses and digest `bfa2cb8cedd57d131329414dc80f85c6db7da1a68eb81143de454bc968d7cf82`.
...
The split reverse join is a complete 103-record bijection to all monolith query hits: `84 exact + 10 image-basename + 9 normalized`. It explicitly maps the two identical monolith `Recursive Sequences` headings to different structural owners instead of using global text membership. All 127 retained records independently close at `98 exact + 13 image-basename + 16 normalized`, with minimum normalized score `0.991511`. Three Index-only context routes point to 26 general-locality or upstream multiregister lines that remain explicitly out-of-scope related controls rather than silently entering T35 mechanics. The flattened Index row at `BOOK:20946` is guarded as the `Carry digits` entry; its nearby `Ceiling` text is a separate column and is not treated as the owner of `in arithmetic, 124`. The exact image interface is 24 candidates = 13 governed (`5/5/3`) + 11 exclusions and matches the asset oracle. The catalog/taxonomy phrase remains search vocabulary absent from the primary Book; five Atlas hits are summary-only. The oracle SHA is `015c18d64b38dadbdfda9197c158f83f2a800fb51a1d7df54bc103ffd1a7a81a`.
