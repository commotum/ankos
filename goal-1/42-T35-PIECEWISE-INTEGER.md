...
Every complete report is bound back to structural program data: 403 transition-event replays contribute to 14,621 total program-bound event replays, two cross-program events and three cross-program traces reject, three horizon-zero traces retain program provenance, and four closed programs have distinct content-derived IDs. Ordered fractions close 100 branch witnesses, an order discriminator, six generic zero-successor `Error(MissingBranch)` results with the last complete state retained, a two-event-then-error trace, 1,004 lazy Conway residue/value checks, and the exact 8,068-event prime-prefix run. Conway's denominator LCM is 6,469,693,230 while the oracle materializes zero rows. A shadowed-row pair proves bare fraction lowering noninjective; the complete ordered source AST/provenance remains required. The oracle also closes 4,101 exact integer-locus cosine/A relations, separates the modulus-30 reachable invariant from positivity, rejects 370 forged witnesses and 64 hostile constructions, and finds no T35 frontier, neighborhood, UPDATE, or executor fields. Its semantic digest is `6057452a9af19cf1f70587ccd640b88f47c4e9dd3b839add88153339262d586a`; its file SHA is `afc482a9e1e21f106371cd8327f4c9d8cef288b6014137b0700296027163d2de`.
