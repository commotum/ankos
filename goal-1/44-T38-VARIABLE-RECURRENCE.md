...
On success, RULE receives the resolved read tree, computes only the exact value, and replaces `End(n)` with `Val(n,next_value) · End(n+1)`; T16's atomic single splice preserves every old value. The strict integer AST cannot produce a nonintegral address: noninteger nodes are construction-invalid. If an evaluated exact address is below the origin, current/future, or otherwise absent, NEIGHBORHOOD/access yields a typed `UndefinedTermReference` failure with the complete old prefix and ordered witness. The common fallible-axis protocol turns that into the zero-successor no-commit `Error`; no RULE write or event exists and no UPDATE commit occurs. This specifies no T38 early-return branch and deliberately leaves the common runner's internal invocation protocol to Goal 2. `Invalid` remains appropriate for malformed AST/type/seed inputs before execution; T26's `Invalid(IncompatibleMosaic)` is an UPDATE-composition failure and is not the T38 analogue.
...
| Invalid demand | 1/2 | Common fallible-axis/no-commit `Error` protocol | Typed `UndefinedTermReference` reason and ordered read-path witness; no RULE write or UPDATE commit; not native halt/default |
...
- Route an actually demanded invalid address through the common fallible-axis/no-commit protocol to `Error(UndefinedTermReference)` with zero successors and no event. Specify no family branch or construction-specific invocation path; add no T38 terminal/outcome class or policy menu.
