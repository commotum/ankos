...
- `22-T03-TOTALISTIC-CA` — REOPENED during T05: the earlier code-357/code-1329/range-two-code-20 repair remains valid, but the included four-color code `1004600` had an unfollowed Notes continuation at `BOOK:19234` and two linked long-run plots. The former 17-query/309-candidate and 116-asset `48/60/8` closure is superseded pending a widened bidirectional audit and fresh review. The semantic result—explicit valuation, fixed-arity equal-weight sum, complete `M=1+(k-1)(2r+1)` table, shared executor, and no new update law—remains active under D115-D118.
