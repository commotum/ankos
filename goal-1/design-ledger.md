...
- Rule/representation: strict binary schemas remain distinct: a complete 512-context positional table; 18 cases `(SelfValue,MooreCount)` at index `2*count+self`; ten all-nine value sums; and the growth restriction fixing all old-black outputs. Their counts are `2^512`, `2^18`, `2^10`, and `2^9`. The generalized profile has index `self+k*sum(neighbors)` and table length `k*((3^d-1)*(k-1)+1)`. Compact tables expand/factor only on complete constant fibers and retain schema-tagged identity. Life `224` is a named value of the 18-case schema; Dyadaxes has the same raw geometry but a lossy majority-gated RULE and remains a different preset, not a substitute.
...
- Consequence: Goal 2 reuses T01/T02/T03/T08/T21 and the branch-free runner, adding Moore access data, closed rule descriptors/factor maps/codecs, named presets, and conformance only. D127 is clarified but no completed stage reopens. No `MooreState`, semantic control class, new UPDATE algebra, executor, rollout branch, hidden center/boundary, Life engine, Dyadaxes shortcut, or raster-defined rule is added.
...
- conflating the 512-context positional, 18-case `(SelfValue,MooreCount)`, ten-case all-nine sum, and nine-free-bit growth schemas; untagged integers, lossy factorization, reversed first/last significance, Book-to-ENU re-sorting without complete table permutation, or symmetric/count fixtures used as orientation evidence;
