...
| RULE writes/replacements | Closed typed results name scalar/label writes, blocks, spans, marker moves, bag/graph replacements, or successor alternatives. T20 tree writes and T37 endpoint writes lower losslessly to ordered spans; T39 finite removals are ordinary `CandidateLocus -> false` writes and its survivor-list operation is a commuting representation lowering. Semantic roles may be tags/products; no control-specific or family-specific result wrapper is required. | ACTIVE RULE axis; declarative query results remain outside rollout | T01, T02, T03, T09, T12, T13, T16, T17, T19, T20, T27, T29, T30, T34, T37, T39, T43, T44; distinct T31/T41/T45 |
| UPDATE composition/schedule | One runner calls a typed UPDATE axis. Implementations compose snapshot finite writes, ordered replacements/splices, bag/graph replacements, and finite successor sets according to explicit schedules/invariants, returning a structured `StepResult` with outcome and event/witness data. T20 tree replacement, T37 endpoint growth, and T39 survivor removal commute with those reusable bases rather than adding implementations or numbered laws. | ACTIVE shared axis; exact implementation sum remains Goal 2 work; stochastic/probabilistic successor measures remain unresolved; not applicable to declarative nonfits | T01, T02, T03, T09, T12, T13, T16, T17, T19, T20, T27, T29, T30, T34, T37, T39, T43, T44; not applicable T31/T41/T45 |
...
- Status: ACTIVE.
- Basis: canonical T17 case (a) has `10->{}` and the bounded count includes words of length zero (`BOOK:12298`, `12308`). T15 independently supplies contextual pair rows that directly emit epsilon in six hash-bound tables. T13, strict T14, and direct T16 evidence require their public outputs to remain nonempty.
- Consequence: private `Word`, ordered-edit, and ordered-emission carriers support `Sigma*`. T17 `ConsumePrefixAppend` and T15 `OrderedEmission` may carry epsilon; T13 `ReplaceOccurrence`, T14 contextual emission, and base T16 `ReplaceInterval` retain `NonEmptyWord` validation. No `allow_empty` flag, silent family-wide broadening, or stage reopening is needed.
...
- Consequence: use a bijective balanced/prefix tagged-token codec in which every subtree is one contiguous span. Prefix-free paths map to disjoint spans, so typed replacements use generic ordered multi-span UPDATE with exact source validation, one old snapshot, context preservation, and lineage. Require `encode(step_tree(s)) = step_spans(encode(s))`; add no tree UPDATE or symbolic executor.
...
- Status: ACTIVE T15 result-schema/preset boundary after exhaustive source, asset, semantic, and architecture closure.
...
- The corrected stage addenda and Goal 2 handoffs supersede the historical integration entries below. The bounded T03/T04 repairs and T06/T07/T08/T10/T11/T14 plus T15 evidence/architecture closure are integrated under active D119-D125. T15 awaits only final hostile/gate closure; T18 follows.
