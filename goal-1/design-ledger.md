...
- Status: ACTIVE T07 property/transform/representation and identity boundary.
...
- Eligibility and predicate: T07 v1 binds one resolved finite deterministic homogeneous `t+1D` CA program `P` to the canonical nonidentity spatial reflection with identity ALPHABET/output action. Its static read schema exposes stable typed `(component,slot,offset)` positions plus a validated component action; reflection closure induces one diagonal complete-read involution `rho`. Equal offsets in distinct components are valid. `Holds` exactly when the ordinary structural evaluator satisfies `T(v)=T(rho v)` for every complete typed read.
...
- Transform and trajectory: the reflected evaluator `T^R=T∘rho` for the scalar catalog action is an ordinary program transform and an involution. `H(Step_P X)=Step_(P^R)(H X)`; a fixed rule commutes with reflection, while a symmetric trajectory additionally requires compatible seed, support realization/boundary, frontier/schedule, and view.
- Representation: for the scalar centered radius-`r` identity-output case, a complete canonical orbit table has `(k^(2r+1)+k^(r+1))/2` rows and is a lossless RULE representation. The physical action is diagonal over the complete typed read, not an independent quotient per component; nontrivial output actions require stabilizer-aware equivariant rows. Representation/provenance, transform, claim, evidence, selection, program, and run identities remain separate.
...
- The corrected stage addenda and Goal 2 handoffs supersede the historical integration entries below. The bounded T03/T04 repairs and T06/T07 are complete, no stage remains reopened, D119-D120 preserve the common architecture, and T08 is next.
...
- `26-T07-SYMMETRIC` — COMPLETE: ten query families, 15 governed continuations, and 74 actual-Index routes close 357 canonical source lines in the exact `15/167/3/98/74` partition; 325 physical split hits reconcile as 313 byte-exact lines plus 12 formatting/OCR variants. Twenty-two physical assets and 44 monolith/split references close at `4 included / 7 relation-only / 11 excluded`. T07 is a class-2 validated reflection property over one resolved CA program; reflected-program transformation is a separate relation, and a complete orbit table is an optional class-3 lossless RULE representation using the same runner. D120 preserves typed positions, diagonal complete-read action, program identity, exact 64 fixed ECA rules, 160 reflection orbits, 88 reflection/color orbits, the 32-rule T06 intersection, source-formula repairs, and seed/boundary/view qualifications. Four embedded checks, independent hostile review, fences/diff gates, and 102 tests pass. No prior stage reopened; next is T08.

## Open Architecture Questions

1. Resolved CA prefix: T01 is the fixed-lattice construction; T02 parameterizes its alphabet/ordered table; T03 adds the exact numeric-sum rule quotient without a second executor; T04/T05 are presets; T06 is a typed uniform-background property restriction; and T07 is a typed reflection-property restriction with separate transform and optional lossless orbit representation. T08 is the next seed-class hypothesis requiring evidence.
