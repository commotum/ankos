...
- Consequence: `OrderedGenerationConcat` accepts snapshot-scoped ordered source handles and exactly one source-bound `Sigma*` emission for every **selected** handle, consumes the old generation, and creates real children in `(source_order,child_ordinal)` order with optional lineage. A zero-length emission remains an inspectable source record and empty interval with no fake child. It never copies unselected old sources forward. T13's `AllOccurrences` and `NonEmptyWord` presets separately enforce their stronger invariants; IDs never enter a rule, and semantic word equality can quotient over order-preserving ID renaming.
