...
5. T14 and T15 share one epsilon-capable private `OrderedGenerationConcat` kernel. Strict T14 validates `Sigma^2->Sigma+`, while T15 validates `Sigma^2->Sigma*` and retains zero-length source witnesses. T14's zero-source empty successor is still not an epsilon row; T15 distinguishes active epsilon, singleton zero-source, and post-extinction empty events from T16/T17 terminals.
