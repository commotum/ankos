...
| RULE writes/replacements | Closed typed results name scalar/label writes, blocks, spans, marker moves, bag/graph replacements, or successor alternatives. T18 phase/head replacement plus old-end insertion, T20 tree writes, and T37 endpoint writes lower losslessly to ordered spans; T39 finite removals are ordinary `CandidateLocus -> false` writes and its survivor-list operation is a commuting representation lowering. Semantic roles may be tags/products; no control-specific or family-specific result wrapper is required. | ACTIVE RULE axis; declarative query results remain outside rollout | T01, T02, T03, T09, T12, T13, T16, T17, T18, T19, T20, T27, T29, T30, T34, T37, T39, T43, T44; distinct T31/T41/T45 |
| UPDATE composition/schedule | One runner calls a typed UPDATE axis. Implementations compose snapshot finite writes, ordered replacements/splices, bag/graph replacements, and finite successor sets according to explicit schedules/invariants, returning a structured `StepResult` with outcome and event/witness data. T18 cyclic spans, T20 tree replacement, T37 endpoint growth, and T39 survivor removal commute with those reusable bases rather than adding implementations or numbered laws. | ACTIVE shared axis; exact implementation sum remains Goal 2 work; stochastic/probabilistic successor measures remain unresolved; not applicable to declarative nonfits | T01, T02, T03, T09, T12, T13, T16, T17, T18, T19, T20, T27, T29, T30, T34, T37, T39, T43, T44; not applicable T31/T41/T45 |
| Successors | UPDATE may yield one configuration, a finite exact set of configurations for multiway rewriting, typed quiescence/reference/empty stutter, a retained zero-successor terminal outcome, or typed no-commit failure. A powerset/layer view is an explicit lift with full witnesses. | ACTIVE runner result algebra; declarative query/model results remain distinct | T01, T02, T03, T09, T12, T13, T16, T17, T18, T19, T20, T27, T29, T30, T34, T37, T39, T43, T44; distinct T31/T41/T45 |
...
- Status: ACTIVE as the shared epsilon-capable ordered-generation base generalized by D124 and D125; T13 retains the stronger full-source-coverage and nonempty-result preset.
- Basis: T13 replaces every old occurrence once and preserves source/block order (`BOOK:1006-1016`, `5944-5952`, `16418-16423`). T14 instead selects exactly the old anchors having a right neighbor, reads overlapping adjacent pairs, and rebuilds the complete successor from their source-ordered emissions (`BOOK:1018-1022`, `12109-12115`). T15's direct rule plates contain epsilon rows under that same schedule; 304,927 bounded cases commute through one generic UPDATE.
- Consequence: `OrderedGenerationConcat` accepts ordered source handles bound to the exact immutable old snapshot and exactly one source-bound `Sigma*` emission for every **selected** handle, consumes the old generation, and creates real children in `(source_order,child_ordinal)` order with optional lineage. Runtime provenance uses an opaque per-snapshot identity; a generation integer is diagnostic metadata and cannot validate a handle because independent runs or branches may share it. Serialized references must carry an explicit run/branch/generation scope (or be rebound and revalidated on load), never a bare `(generation,index)` identity. A zero-length emission remains an inspectable source record and empty interval with no fake child. It never copies unselected old sources forward. T13's `AllOccurrences` and `NonEmptyWord` presets separately enforce their stronger invariants; IDs never enter a rule, and semantic word equality can quotient over order-preserving ID renaming.
...
- The corrected stage addenda and Goal 2 handoffs supersede the historical integration entries below. The bounded T03/T04 repairs and T06/T07/T08/T10/T11/T14/T15/T18/T21/T22 closures are integrated under active D119-D128. Next: T23.
