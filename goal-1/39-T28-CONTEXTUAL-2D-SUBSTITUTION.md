...
6. Compile valid clauses to the canonical exhaustive context table for execution, but retain the ordered source AST and provenance. If the declared product-pattern importer promises Wolfram `ReplaceAll` whole-context nonmatch semantics, make every fallback row/template explicit during import.
...
- No compact pattern reordered or treated as an unordered map; the source's ordered first-match behavior is preserved. Missing coverage is rejected during program construction unless the explicit product-pattern importer has materialized its promised whole-context `ReplaceAll` nonmatch behavior as closed total data.
