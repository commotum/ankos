...
def expand_compatible_mosaic(
    ...
    assert grid and all(grid) and len({len(row) for row in grid}) == 1
    height = len(grid)
    assert rule and {value for row in grid for value in row} <= set(rule)
    patches = tuple(
...
def main() -> int:
    ...
    coordinate_integerdigits_defect_ok = (
        "IntegerDigits[{i, i}, k, n]" in at(13695)
        and "\\{j, 0, k^n - 1\\}" in at(13696)
        and {13695, 13696} <= RELATION_EVIDENCE
        and not ({13695, 13696} & NATIVE_EVIDENCE)
    )
    ok &= coordinate_integerdigits_defect_ok
    print(
        "source_coordinate_integerdigits_defect_retained_as_relation_not_repaired",
        "OK" if coordinate_integerdigits_defect_ok else "MISMATCH",
    )
    ...
    crossed_width_grid = ((0, 1), (1, 0))
    try:
        crossed_width_result = expand_compatible_mosaic(
            crossed_width_grid,
            crossed_width_rule,
        )
    except AssertionError:
        crossed_width_result = ()
    crossed_width_compatibility_ok = (
        crossed_width_result == ((0, 1, 1), (1, 1, 0))
        and {
            len(crossed_width_rule[crossed_width_grid[row][0]][0])
            for row in range(2)
        }
        == {1, 2}
        and {
            len(crossed_width_rule[crossed_width_grid[row][1]][0])
            for row in range(2)
        }
        == {1, 2}
        and {len(row) for row in crossed_width_result} == {3}
    )
    ok &= crossed_width_compatibility_ok
    print(
        "derived_flatten2d_accepts_crossed_column_widths",
        "OK" if crossed_width_compatibility_ok else "MISMATCH",
        "2x3",
        "both_columns=1,2",
    )
    ...
    architecture_inference_ok = (
        ...
        and coordinate_integerdigits_defect_ok
        and other_shapes_literal_repair_ok
        and background_dimension_ok
        and mixed_native_ok
        and crossed_width_compatibility_ok
