...
| D128 / T22 | 264-line source closure; 95-asset candidate closure at 68 governed + 27 adjacency-only; four source-pinned 512/18/10/9-case RULE schemas; 1,417 independent native/generic commutations; 4,608 Book/ENU context cases, 512 table-basis permutation cases, and all 19,683 ternary contexts/fibers | 3 | D127 fixed-square-grid CA preset + T02 complete local maps + T03 factorized RULE representations + T08 configurations | exactly one declared Self plus eight ordered Moore offsets; complete coordinate-basis/table permutation; compact fibers factor only qualifying exhaustive maps; quotient reads preserve slot multiplicity; Life `224` is the `B3/S23` named preset; finite-`k,d` table formula is validated | Add Moore access/RULE presets, codecs, and named fixtures only; no stage reopens and no state class, new UPDATE, executor, family branch, Life family, hidden center/boundary, or raster semantics is added |
...
- T22 subsequently parameterizes the same fixed-lattice preset by explicit Self-plus-eight Moore access and schema-tagged 512/18/10/9-case RULE data. Complete table/fiber and coordinate permutations commute, small quotients preserve read-slot multiplicity, and Life `224` remains an ordinary `B3/S23` preset. D128 adds no UPDATE law, family class, or executor.
...
| T22 | Discrete `t+2D`; T21's total finite-alphabet field and fixed square-grid `Z^2` configuration unchanged | All sites; exactly one declared Self plus eight unique ordered Moore offsets in a tagged coordinate frame; quotient aliases retain slot multiplicity | One same-site label write; T01 snapshot-parallel UPDATE. General 512-context, outer-totalistic 18-case, all-nine-sum 10-case, and growth-restricted 9-bit RULE schemas stay distinct with certified factor/codec maps; Life `224` is a named preset | Add Moore access data, schema-tagged RULE representations/codecs, coordinate-table permutation, and conformance fixtures; no Moore/Life state, UPDATE, executor, family branch, Dyadaxes substitution, implicit center/boundary, or raster rule |
