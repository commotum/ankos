...
| D059 | Center-conditioned neighbor-count evidence | 3 | Closed local-relation AST on a declared lattice DOMAIN | Complete allowed sets; offsets distinct; no predicate callback | Keep as relation node, not class/executor |
...
| D081 | Ulam first-accepted selection plus append | 1 | Program-coupled FRONTIER/RULE feeding shared endpoint UPDATE | Complete prefix read; distinct unordered indices; accepted witness explicit | Keep composition; remove dependency on numbered ninth law |
...
| T20 | Discrete recursive-tree DOMAIN/topology with tagged expression labels; no ambient Euclidean dimension is asserted | Ordered maximal prefix-free matches; matched subtrees/bindings | Replacement subtrees; atomic prefix-free structural UPDATE | Add tree topology/path selectors and closed pattern/template rules inside runner; no symbolic executor |
...
| T29 | Discrete graph DOMAIN/topology; rooted port-labeled graph; no ambient Euclidean dimension is asserted | Old vertices; port-path/reach views | Direct/fresh reroutes; create/rewire/root-project UPDATE | Add graph domain/topology, structural access, and graph update policy within runner; no network executor |
...
| T39 | Discrete ordered candidate support with visible stage marker | Current stage/divisor marker locus; candidate membership and proper-multiple witnesses | Remove selected survivors and advance marker atomically | Add stage-marker selector, candidate-set access, and typed subset/marker UPDATE policy; pure filters/measurements stay queries |
