...
| D130 / T24 | 142-line query union and 145-line retained source closure at `80 native / 16 relation / 49 control`; bounded radius-four asset universe at 11 governed + 11 adjacency-only, 22 hashes, and `22 hash-bound / 6 transcribed / 0 pixel-replayed`; 1,689 native/generic events; arbitrary-`d` formulas and 4D width-81 closed positional data; four coefficient-basis commutations; exact hex `C6/D6` orbit counts 28/26 and 1,024 source-code events; typed alternating/two-shape/finite-type incidence; unlabeled exact-sum permutation checks; canonical labeled-slot reads; alias/parallel-port multiplicity; old-snapshot adversary | 1/2/3 | D127-D129 fixed-lattice CA preset + T02 complete local maps + T03 compact/orbit RULE representations + T08 configurations + immutable fixed-incidence support/access | DOMAIN is `t+dD` only when spatial dimension is native; abstract fixed networks are `t+0D`; CONFIGURATION owns immutable incidence/site kinds/ports; basis/motif/staggered codecs are lossless representations; strict unlabeled access factors through the neighbor-only exact numeric total, while positional access uses declared semantic slots rather than storage order; repeated incidence occurrences survive aliases; finite kind-indexed tables are closed data rather than family dispatch | Add generic dimension/SiteRef, immutable fixed incidence, topology-aware AllSites, kind-indexed access/RULE sums, neighbor-only exact totals, canonical slot validation, exact wide keys, and realization/view codecs; keep same-site snapshot UPDATE; no stage reopens and no state/control class, new UPDATE, executor, family branch, hidden topology/embedding, callback, or raster semantics is added |
...
| T24 | Discrete `t+dD` for native embedded dimension; immutable fixed incidence/site kinds/ports in CONFIGURATION; abstract unembedded fixed networks use `t+0D`; finite quotients, basis/motif/staggered codecs, layouts, and rasters remain realizations/representations/views | All sites; semantic basis-offset slots, kind-indexed ports, or explicit incidence occurrences; strict unlabeled connections expose neighbor-only exact numeric-total reads, positional reads are canonicalized by declared semantic slots, and aliases retain multiplicity | One same-site label write; T01 snapshot-parallel UPDATE. General positional, neighbor-only total, explicit-Self axis/full aggregate, hex orbit/outer/total/growth, and finite kind-indexed tables are distinct closed schema-tagged data; fixed-network labels do not invoke T29 structural writes | Add generic discrete dimension/SiteRef, immutable typed incidence, topology-aware AllSites/access, closed dependent rule sums, neighbor-only exact-total and canonical-slot validation, exact wide keys, and conformance; no T24 state/control, UPDATE, executor, family branch, hidden topology/embedding, callback, eager giant table, or raster rule |
