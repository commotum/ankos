...
■ **Page 587 · Random block sequences.** Analytical forms for all but the last spectrum are: 1,  $u^2/(1+8u^2)$ ,  $1/(1+8u^2)$ ,  $u^2$ ,  $(1-4u^2)^2/(1-5u^2+8u^4), u^2/(1-5u^2+8u^4),$  $u^2 + 1/36 \, \text{DiracDelta}[\omega - 1/3]$ , where  $u = \text{Cos}[\pi \, \omega]$ , and  $\omega$ runs from 0 to 1/2 in each plot. Given a list of blocks such as {{1, 1}, {0}} each element of Flatten[list] can be thought of as a state in a finite automaton or a Markov process (see page 1084). The transitions between these states have probabilities given by m[Map[Length, list]] where
...
■ **Page 607 · Implementation.** The color of a cell at position `{x, y}` in the pattern shown is given  `Extract[{{1, 0, 1}, {0, 1, 0}}, Mod[{y, x}, {2, 3}] + 1]`.
