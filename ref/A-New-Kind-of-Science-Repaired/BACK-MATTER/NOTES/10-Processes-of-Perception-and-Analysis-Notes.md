...
For any substitution system the spectrum  $\phi[i][t, \omega]$  at step t from initial condition i is given by a linear recurrence relation in terms of the  $\phi[i][t-1, \omega]$ . With k colors each giving a string of the same length s the recurrence relation is
...
- (d) (Period-doubling sequence) The spectrum  $(2^{\#}-(-1)^{\#} \&)[1+IntegerExponent[n, 2]]$ , almost like the markings on a base 2 ruler.
