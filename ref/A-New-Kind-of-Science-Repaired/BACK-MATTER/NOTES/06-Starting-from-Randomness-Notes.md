...
and a measure spacetime entropy  $h_{tx}^{\mu}$  by replacing s with p Log[p]. In general,  $h_t \le h_{tx} \le 2 \lambda h_x$  and  $h_x \le 2r h_t$ . For additive rules like rule 90 and rule 150 every possible configuration of the initial block leads to a different configuration for the patch, so that  $h_{tx} = 2r = 2$ . But for other rules many different configurations of the initial block can lead to the same configuration for the patch, yielding potentially much smaller values of  $h_{tx}$ . Just as for most other entropies, when a cellular automaton shows complicated behavior it tends to be difficult to find much more than upper bounds for  $h_{tx}$ . For rule 30,  $h_{tx}^{\mu} < 1.155$ , and there is some evidence that its true value may actually be 1. For rule 18 it appears that  $h_{tx}^{\mu} = 1$ , while for rule 22,  $h_{tx}^{\mu} < 0.915$  and for rule 54, $h_{tx}^{\mu} < 0.25$.
