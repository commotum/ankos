...
Keep the coverage catalog distinct from semantic families. A new catalog row may be a preset, property, seed class, composition, declarative category, or other Book obligation inside an existing family. Same-family claims require either a lossless complete-state/denotation correspondence preserving native results, or proof that both entries instantiate the same explicit parameterized schema with unchanged mechanics. A shared five-field API, opaque packing, callback, hidden phase/history, or multi-step emulation is not semantic identity. Only new-family claims require a concrete counterexample to the nearest existing family.
