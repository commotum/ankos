...
- `GENERATED_LINK` destinations are computed, never caller-authored: start with the manifest-declared canonical source document parent and target release path, compute their POSIX lexical relative path, and optionally append one validated reserved `ankos-` fragment. `..` components are allowed only when component-wise normalization from the source parent never escapes the repaired root and resolves exactly to the declared target; this is required for links such as Notes-to-Chapter backlinks. Backslash, percent escapes, control characters, symlink traversal, an undeclared target, and repaired-root escape are rejected. Emit `[label](destination)` with delimiters/destination typed generated. `SOURCE_LINK` emits only from its protected `source_syntax_projection`; the parsed label, destination, and title must rejoin that projection exactly.
