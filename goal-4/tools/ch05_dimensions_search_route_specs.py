...
def route(
    ...
    *,
    ...
    if attempts is None:
        if status == "RESOLVED":
            attempts = (
                "Resolved within the assigned Chapter 5 main/Notes sources "
                "by the Stage 9 local-search route audit; closure is bounded "
                "to the listed units and assets.",
            )
        else:
            attempts = (
                "Queued by the Stage 9 local search; the target lies outside "
                "the assigned Chapter 5 source range and was not opened.",
            )
    return {
        "route_id": "",
